...
function handleLogout() {
    if (confirm('Are you sure you want to logout?')) {
        ...
        currentUser = null;
        userBalance = { BTC: 0, USDT: 5000, ETH: 0 };
        transactions = [];
        document.getElementById('loginForm').reset();
        document.getElementById('signupForm').reset();
        showLandingPage();
...
function processWithdraw() {
    ...
    BalanceManager.saveBalance(userBalance); // SAVE AFTER WITHDRAWAL
    updateAllBalances();
...
function handleConvert() {
    ...
    BalanceManager.saveBalance(userBalance); // SAVE AFTER CONVERSION
    updateAllBalances();
...
function applyPromoCode() {
    ...
    if (code === PROMO_CODE) {
        ...
        BalanceManager.saveBalance(userBalance); // SAVE AFTER PROMO APPLIED
        updateAllBalances();
